# Repository Index

> **Version:** 2.4.0 | **Last Updated:** 26 March 2026

Master navigation for the Claude Prompt Engineering Guide repository.

---

## Core Documentation

| File | Description |
|------|-------------|
| [README.md](./README.md) | Main repository introduction and quick links |
| [Claude-Prompt-Guide.md](./Claude-Prompt-Guide.md) | Comprehensive prompt engineering guide |
| [ultimate.claude](./ultimate.claude) | Master system prompt for Claude projects |
| [CHANGELOG.md](./CHANGELOG.md) | Version history and release notes |
| [CONTRIBUTING.md](./CONTRIBUTING.md) | Contribution guidelines |
| [LICENSE](./LICENSE) | MIT License |
| [MIGRATION-NOV2025-JAN2026.md](./MIGRATION-NOV2025-JAN2026.md) | Migration guide for Nov 2025 to Jan 2026 |

---

## Guides (`docs/`)

| File | Description |
|------|-------------|
| [quick-start.md](./docs/quick-start.md) | Get started in 5 minutes |
| [api-guide.md](./docs/api-guide.md) | Claude API integration guide |
| [claude-code-guide.md](./docs/claude-code-guide.md) | Claude Code CLI tool guide |
| [cowork-guide.md](./docs/cowork-guide.md) | Claude Cowork autonomous workflows |
| [mcp-integration.md](./docs/mcp-integration.md) | Model Context Protocol setup |
| [mcp-ecosystem-overview.md](./docs/mcp-ecosystem-overview.md) | MCP ecosystem guide (Jan 2026) |
| [skills-guide.md](./docs/skills-guide.md) | Skills documentation and patterns |
| [superpowers-guide.md](./docs/superpowers-guide.md) | Superpowers plugin guide |
| [hooks-guide.md](./docs/hooks-guide.md) | Claude Code Hooks (24 events) reference |
| [agent-teams-guide.md](./docs/agent-teams-guide.md) | Agent Teams multi-agent coordination |
| [ultrathink-thinking-modes.md](./docs/ultrathink-thinking-modes.md) | Ultrathink, adaptive thinking, effort levels |
| [antigravity-comparison.md](./docs/antigravity-comparison.md) | Google Antigravity vs Claude Code |
| [messenger-integrations.md](./docs/messenger-integrations.md) | WhatsApp, Telegram, Slack, Discord integrations |
| [motion-graphics-claude-remotion-guide.md](./docs/motion-graphics-claude-remotion-guide.md) | Remotion video generation with Claude Code |
| [healthcare-compliance.md](./docs/healthcare-compliance.md) | HIPAA/healthcare integration |
| [plugins-guide.md](./docs/plugins-guide.md) | Plugins ecosystem, marketplace, creating plugins |
| [claude-md-guide.md](./docs/claude-md-guide.md) | CLAUDE.md configuration, rules files, auto-memory |
| [subagents-guide.md](./docs/subagents-guide.md) | Subagent tool, model routing, parallel execution |
| [claude-code-tips-and-tricks.md](./docs/claude-code-tips-and-tricks.md) | 37 power user tips and hidden features |
| [skills-catalog.md](./docs/skills-catalog.md) | Curated catalog of 100+ skills across 15 categories |

### Research & Analysis (`docs/`)

| File | Description |
|------|-------------|
| [research-report-jan-2026.md](./docs/research-report-jan-2026.md) | Institutional-grade research report (8,500+ words) |
| [ecosystem-market-analysis.md](./docs/ecosystem-market-analysis.md) | Market position & growth analysis |
| [pricing-comparison-jan-2026.md](./docs/pricing-comparison-jan-2026.md) | Claude vs GPT-4 vs Gemini pricing |
| [research-opencode-clawbot-jan-2026.md](./docs/research-opencode-clawbot-jan-2026.md) | OpenCode CLI, AirLLM, external agents |

### Planning Templates (`planning/`)

| File | Description |
|------|-------------|
| [task.md](./planning/task.md) | Living backlog for Claude projects |
| [progress.md](./planning/progress.md) | Daily/weekly session log |
| [activity.md](./planning/activity.md) | Chronological action timeline |

### Examples (`docs/examples/`)

| File | Description |
|------|-------------|
| [coding-tasks.md](./docs/examples/coding-tasks.md) | Code review and development prompts |
| [business-analysis.md](./docs/examples/business-analysis.md) | Business and financial analysis |
| [research-tasks.md](./docs/examples/research-tasks.md) | Research and information gathering |
| [document-creation.md](./docs/examples/document-creation.md) | Documentation and writing tasks |

---

## Templates (`templates/`)

| File | Description |
|------|-------------|
| [minimal-prompt-template.md](./templates/minimal-prompt-template.md) | Quick, lightweight template |
| [comprehensive-prompt-template.md](./templates/comprehensive-prompt-template.md) | Full 10-component template |
| [example-clauderules.md](./templates/example-clauderules.md) | Self-evolving CLAUDE.md pattern |

---

## Skills (`skills/`)

| File | Description |
|------|-------------|
| [README.md](./skills/README.md) | Skills guide and documentation |
| [README.ja.md](./skills/README.ja.md) | Japanese translation |
| [README.zh-CN.md](./skills/README.zh-CN.md) | Chinese translation |

### Skill Examples (`skills/examples/`)

#### Development Skills

| Skill | Description |
|-------|-------------|
| [nextjs-app-router-skill.md](./skills/examples/nextjs-app-router-skill.md) | Next.js 15 App Router patterns |
| [typescript-standards.md](./skills/examples/typescript-standards.md) | TypeScript coding standards |
| [api-development-skill.md](./skills/examples/api-development-skill.md) | REST/GraphQL API development |
| [prisma-orm-skill.md](./skills/examples/prisma-orm-skill.md) | Prisma ORM patterns |
| [tailwind-design-system-skill.md](./skills/examples/tailwind-design-system-skill.md) | Tailwind CSS design systems |

#### Testing Skills

| Skill | Description |
|-------|-------------|
| [testing-skill.md](./skills/examples/testing-skill.md) | General testing patterns |
| [vitest-unit-testing-skill.md](./skills/examples/vitest-unit-testing-skill.md) | Vitest unit testing |
| [playwright-e2e-testing-skill.md](./skills/examples/playwright-e2e-testing-skill.md) | Playwright E2E testing |

#### DevOps & Infrastructure

| Skill | Description |
|-------|-------------|
| [aws-cloud-infrastructure-skill.md](./skills/examples/aws-cloud-infrastructure-skill.md) | AWS EC2/RDS deployment |
| [google-cloud-platform-skill.md](./skills/examples/google-cloud-platform-skill.md) | GCP infrastructure |
| [vercel-deployment-skill.md](./skills/examples/vercel-deployment-skill.md) | Vercel deployment |
| [neon-serverless-skill.md](./skills/examples/neon-serverless-skill.md) | Neon serverless Postgres |
| [monitoring-logging-skill.md](./skills/examples/monitoring-logging-skill.md) | Observability patterns |

#### Security & Quality

| Skill | Description |
|-------|-------------|
| [code-review.md](./skills/examples/code-review.md) | PR review workflow |
| [security-compliance.md](./skills/examples/security-compliance.md) | Security standards |
| [nextauth-authentication-skill.md](./skills/examples/nextauth-authentication-skill.md) | NextAuth.js authentication |

#### Other Skills

| Skill | Description |
|-------|-------------|
| [git-workflow-skill.md](./skills/examples/git-workflow-skill.md) | Git branching and workflows |
| [database-migrations.md](./skills/examples/database-migrations.md) | Database migration patterns |
| [performance-optimization-skill.md](./skills/examples/performance-optimization-skill.md) | Performance tuning |
| [seo-optimization-skill.md](./skills/examples/seo-optimization-skill.md) | SEO best practices |
| [accessibility-ux.md](./skills/examples/accessibility-ux.md) | Accessibility and UX |
| [example-feedback-analyzer.md](./skills/examples/example-feedback-analyzer.md) | Customer feedback analysis |

---

## Translations

| Language | README | Skills README |
|----------|--------|---------------|
| English | [README.md](./README.md) | [skills/README.md](./skills/README.md) |
| Japanese | [README.ja.md](./README.ja.md) | [skills/README.ja.md](./skills/README.ja.md) |
| Chinese | [README.zh-CN.md](./README.zh-CN.md) | [skills/README.zh-CN.md](./skills/README.zh-CN.md) |

---

## Quick Links

- **New to Claude?** Start with [Quick Start Guide](./docs/quick-start.md)
- **Building an app?** Read [API Integration Guide](./docs/api-guide.md)
- **Using Claude Code?** See [Claude Code Guide](./docs/claude-code-guide.md)
- **Hooks?** See [Hooks Guide](./docs/hooks-guide.md)
- **Agent Teams?** See [Agent Teams Guide](./docs/agent-teams-guide.md)
- **Ultrathink?** See [Thinking Modes Guide](./docs/ultrathink-thinking-modes.md)
- **Need skills?** Browse [Skills Catalog](./docs/skills-catalog.md) (100+ curated)
- **Plugins?** See [Plugins Guide](./docs/plugins-guide.md)
- **Subagents?** See [Subagents Guide](./docs/subagents-guide.md)
- **CLAUDE.md?** See [CLAUDE.md Guide](./docs/claude-md-guide.md)
- **Tips & Tricks?** See [Power User Tips](./docs/claude-code-tips-and-tricks.md)
- **Migrating?** Check [Migration Guide](./MIGRATION-NOV2025-JAN2026.md)
- **Market Analysis?** Read [Ecosystem Market Analysis](./docs/ecosystem-market-analysis.md)
- **Pricing Info?** See [Pricing Comparison Guide](./docs/pricing-comparison-jan-2026.md)
- **MCP Integration?** Check [MCP Ecosystem Overview](./docs/mcp-ecosystem-overview.md)
- **Alternative Tools?** See [OpenCode/AirLLM Research](./docs/research-opencode-clawbot-jan-2026.md)
- **Project Planning?** Use [Planning Templates](./planning/)

---

**Repository:** Claude Prompt Engineering Guide
**Version:** 2.4.0
**Updated:** 26 March 2026
